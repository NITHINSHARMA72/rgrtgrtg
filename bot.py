...
def send_channel_join_prompt(chat_id):
    ...
    text = (
        ...
        f"👉 {CHANNEL_URL}\n\n"
        "Channel join karne ke baad niche **'Verify Membership'** button par click karein! 🚀"
    )
...
def verify_callback(call):
    ...
    if check_channel_membership(user_id):
        ...
        bot.edit_message_text(
            "🎉 **Awesome!** Aapne channel join kar liya hai. Ab aap bot ko freely use kar sakte hain. Kuch bhi type karke baat shuru karein! 🚀",
...
def handle_voice(message):
    ...
    try:
        ...
        bot.reply_to(
            message,
            "😅 Ops! Voice process karne mein thodi problem aayi. Text message bhej kar try karein.",
        )
    finally:
        for f in [
            "voice_msg.ogg",
